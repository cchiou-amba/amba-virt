...
static void __maybe_unused amba_uart_doorbell_ack(struct amba_uart_port *amb_port)
{
	if (amb_port && amb_port->doorbell_base) {
		/* Write to Doorbell register: Peer 1 (Host Server), Vector 0 */
		writel_relaxed((1U << 16) | 0U, amb_port->doorbell_base + 0x0c);
	}
}
...
static void amba_uart_hw_init(struct uart_port *port)
{
	...
	/* Disable interrupts (FIFO polling mode) */
	amba_uart_write(port, 0x00, UART_IE_OFFSET);
}
...
static int amba_uart_startup(struct uart_port *port)
{
	...
	amb_port->poll_interval = ms_to_ktime(1); /* 1 ms polling interval */
