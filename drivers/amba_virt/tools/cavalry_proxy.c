...
int cavalry_proxy_init(int fd_amba_virt, unsigned char *shm_map, size_t shm_size)
{
	...
	if (ioctl(g_fd_cav, CAVALRY_GET_CAVALRY_STATUS, &status) < 0) {
		perror("cavalry_proxy: get cavalry status");
	}

	printf("cavalry_proxy: initialized (cav_fd=%d, window_fd=%d, chip=%u, started=%u)\n",
			   ...
			   uint32_t client_cid)
{
	...
	if (g_shm_map == MAP_FAILED || !g_shm_map) {
		resp->status = -ENODEV;
		return 0;
	}

	/* Deep-copy the request out of the shared BAR into host heap */
	host_arena_copy = malloc(req->arena_len);
	if (!host_arena_copy) {
		resp->status = -ENOMEM;
		return 0;
	}
	memcpy(host_arena_copy, g_shm_map + CAVALRY_RPC_ARENA_OFFSET, req->arena_len);
	...
	for (d = 0; d < run_req->dag_cnt; d++) {
		...
		if (ret < 0) {
			fprintf(stderr, "cavalry_proxy: invalid DVI address 0x%lx size %u\n",
				(unsigned long)dag->dvi_dram_addr, dag->dvi_img_size);
			resp->status = ret;
			goto out_free_arena;
		}
		...
		for (p = 0; p < dag->port_cnt; p++) {
			ret = cavalry_proxy_validate_range(dag->port_desc[p].port_dram_addr,
							   ...
							   client_cid);
			if (ret < 0) {
				fprintf(stderr, "cavalry_proxy: invalid port [%u-%u] addr 0x%lx size %lu\n",
	...
	/* AMA Dynamic CMA command allocation pattern (kernel allocates in AMA) */
	mfd->ucode_cmd_addr_fd = -1;
	mfd->ucode_cmd_addr_offset = 0;
	mfd->dvi_dram_addr_fd = g_window_fd;
	...
	for (d = 0; d < run_req->dag_cnt; d++) {
		...
		/* Descriptors and poke offsets relative to window_fd */
		...
		for (p = 0; p < src->port_cnt; p++) {
			dst->port_dram_addr_fd[p] = g_window_fd;
			dst->port_desc[p].port_dram_addr = src->port_desc[p].port_dram_addr;
		}
	}

	/* Dispatch ioctl to real host /dev/cavalry */
			       ...
			       uint32_t client_cid)
{
	...
	if (ret < 0) {
		fprintf(stderr, "cavalry_proxy: register invalid staging range off=0x%x size=%u\n",
			reg_desc->staging_bar_offset, reg_desc->staging_size);
		resp->status = ret;
		goto out_free_arena;
	}
	...
	mem.cache_en = 0;
	if (ioctl(g_fd_cav, CAVALRY_ALLOC_MEM, &mem) < 0) {
	...
	/* Deep-copy staging slice from guest BAR into host-private AMA */
	memcpy(host_virt, g_shm_map + src_base, reg_desc->staging_size);
				     ...
				     uint32_t client_cid)
{
	...
	for (p = 0; p < run_reg->port_cnt && p < CAVALRY_MAX_PORTS; p++) {
		...
		if (h_ret < 0) {
			fprintf(stderr, "cavalry_proxy: handle lookup failed for hid=%u (ret=%d)\n",
				bind->handle_id, h_ret);
			resp->status = h_ret;
			goto out_free;
		}
		...
		for (d = 0; d < mfd->dag_cnt; d++) {
			if (p_idx < mfd->dag_desc[d].port_cnt) {
				mfd->dag_desc[d].port_dram_addr_fd[p_idx] = g_window_fd;
				mfd->dag_desc[d].port_desc[p_idx].port_dram_addr = hend.bar_offset + bind->offset;
			     ...
			     uint32_t client_cid)
{
	...
	case VCAV_OP_GET_VERSION: {
		...
		if (ioctl(g_fd_cav, CAVALRY_GET_DRIVER_VERSION, &ver) < 0)
			resp->status = -errno;
		else
			resp->status = 0;
		break;
	...
	case VCAV_OP_GET_CHIP_ID: {
		uint32_t chip_id = 0;
		if (ioctl(g_fd_cav, CAVALRY_GET_CV_CHIP_ID, &chip_id) < 0)
			resp->status = -errno;
		else {
	...
	case VCAV_OP_GET_STATUS: {
		...
		if (ioctl(g_fd_cav, CAVALRY_GET_CAVALRY_STATUS, &status) < 0) {
			...
			printf("cavalry_proxy: GET_STATUS ioctl returned started=%u, raw0=0x%08x\n",
			       status.is_cavalry_started, *(uint32_t *)&status);
		}
		break;
	}
	...
	case VCAV_OP_QUERY_UCODE_CMD_SIZE: {
		...
		if (ioctl(g_fd_cav, CAVALRY_QUERY_UCODE_CMD_SIZE, &ucmd) < 0) {
			resp->status = -errno;
		} else {
			resp->status = 0;
